...
func (c *Converter) ConvertWebMToPCM(webmData []byte) ([]byte, error) {
	...
	if err != nil {
		fmt.Printf("⚠️ FFmpeg conversion failed: %v\n", err)
		fmt.Printf("⚠️ Falling back to simulation (this will generate LARGE data)\n")
		// Fallback to simulation for demo purposes
		return c.simulateWebMToPCMConversion(webmData)
	}

	fmt.Printf("✅ FFmpeg conversion successful: %d WebM bytes -> %d PCM bytes\n", len(webmData), len(pcmData))
	return pcmData, nil
}
...
func (c *Converter) simulateWebMToPCMConversion(webmData []byte) ([]byte, error) {
	...
	// More realistic estimate: WebM is compressed ~8:1 for short audio clips
	estimatedSamples := len(webmData) * 8
	if estimatedSamples > c.sampleRate*10 { // Limit to 10 seconds max
		estimatedSamples = c.sampleRate * 10
	}
	if estimatedSamples < c.sampleRate/10 { // Minimum 0.1 second (was 1 second!)
		estimatedSamples = c.sampleRate / 10
	}
